...
The model with the reduced set ('rf_bor') has a higher accuracy (0.78) compared to the model with all covariates ('rf_basic') with an accuracy of 0.76. 
The Kappa statistic for the model with the reduced set is also higher (0.5228) compared to the model with all covariates (0.4794), indicating a higher level of agreement beyond chance.
...
```
A lower OOB error generally indicates better generalization to unseen data. In this comparison, the model with the reduced set of predictors (rf_bor) exhibits a lower OOB error (0.1438674) compared to the model with all covariates (rf_basic, 0.1503242). Therefore, considering both the performance metrics on the test set and the OOB error, the model with the reduced set informed by Boruta (rf_bor) is a preferred choice.
...
Mod_best demonstrates a slightly higher accuracy (0.775) and enhanced sensitivity (0.8231) compared to rf_basic (accuracy: 0.76, sensitivity: 0.8000). In terms of specificity, mod_best also shows a marginal improvement with a value of 0.7143, whereas rf_basic has a specificity of 0.6857. This suggests comparable proficiency in correctly identifying negative instances, with mod_best showcasing a slight edge.

The Kappa statistic further supports the enhanced performance of mod_best, recording a value of 0.5228 compared to rf_basic's 0.4794. This indicates that mod_best achieves a higher level of generalization to unseen data. Additionally, mod_best achieves a slightly higher balanced accuracy of 0.7648, reinforcing the marginal but consistent improvement in the optimized model.
